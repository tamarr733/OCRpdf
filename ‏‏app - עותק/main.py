...
@app.post("/ocr/test-tesseract")
async def test_tesseract():
    """Test if tesseract works at all"""
    try:
        ...
        logger.info(f"Available languages:\n{result.stdout}")
...
async def ocr_pdf_overlay(file: UploadFile = File(...)):
    ...
    try:
        ...
        ocrmypdf.ocr(
            ...
            output_dpi=300,
            # clean=True,       
            # deskew=False,      # Changed from True
            # rotate_pages=False, # Changed from True
            lang="eng",
            # force_ocr=True,   # Changed from True - THIS IS KEY
            jobs=1,           
            # progress_bar=False,
            redo_ocr=True,      # Remove existing OCR/text
            clean=True,
            verbose=2,
